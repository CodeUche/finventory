...
class BudgetService:
    ...
    def _actual_for_line(line, budget, org):
        ...
        Matches by the ExpenseCategory FK when the line has one, otherwise
        falls back to a case-insensitive category-name match (legacy data
        with no FK). Either way, expenses are only counted if they are
        EITHER explicitly linked to THIS budget via Expense.budget, OR not
        linked to any budget at all (today's plain category-match
        behaviour, preserved for expenses nobody has tagged yet).
