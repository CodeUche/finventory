...
from apps.core.permissions import (
    IsManager, IsStaff, IsOwnerOrAdmin, plan_requires, requires_module,
)

# The owner's per-person ticks, enforced server-side (H-2). 'leave' is a
# separate module from 'payroll' by design, so a line manager can approve
# time off without being able to see anyone's salary.
_HRAccess = requires_module("payroll")
_LeaveAccess = requires_module("leave")
...
class EmployeeViewSet(ExportMixin, TenantFilterMixin, viewsets.ModelViewSet):
    ...
    permission_classes = [IsAuthenticated, IsStaff, _PlanPayroll, _HRAccess]
...
class PayrollRunViewSet(TenantFilterMixin, viewsets.ModelViewSet):
    ...
    permission_classes = [IsAuthenticated, IsManager, _PlanPayroll, _HRAccess]
...
class EmployeeTaxProfileViewSet(TenantFilterMixin, viewsets.ModelViewSet):
    ...
    permission_classes = [IsAuthenticated, IsOwnerOrAdmin, _HRAccess]
...
class LeaveCarryForwardViewSet(TenantFilterMixin, viewsets.ViewSet):
    ...
    """
    permission_classes = [IsAuthenticated, IsOwnerOrAdmin, _PlanPayroll, _LeaveAccess]
